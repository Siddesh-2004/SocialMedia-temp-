...
export const usersTable = table(
  ...
  (table) => [
    t.check(
      "fullName_check",
      sql`length(${table.fullName}) >= 2 AND length(${table.fullName}) <= 100`,
    ),
    t.check(
      "userName_check",
      sql`length(${table.userName}) >= 2 AND length(${table.userName}) <= 100`,
    ),
    t.check(
      "password_check",
      sql`length(${table.password}) >= 6 AND length(${table.password}) <= 100`,
    ),
  ],
);
